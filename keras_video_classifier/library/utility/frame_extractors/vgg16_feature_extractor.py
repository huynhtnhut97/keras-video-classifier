...
def extract_vgg16_features_live(model, video_input_file_path, feature_dir_name ):
	print('Extracting frames from video: ', video_input_file_path)
	feature_output_file_path = feature_dir_name + os.path.sep + os.path.splitext(os.path.basename(video_input_file_path))[0]+ '.npy'
	if os.path.exists(feature_output_file_path):
		print("Found {}".format(os.path.basename(feature_output_file_path)))
...
def extract_vgg16_features_live_each_interval(model, video_input_file_path, feature_dir_name, interval):
	feature_output_file_path = feature_dir_name + os.path.sep + os.path.splitext(os.path.basename(video_input_file_path))[0]
	print("Finding {}".format(feature_output_file_path))
	if os.path.exists(feature_output_file_path):
		with open(feature_output_file_path, 'rb') as pickle_file:
	...
	while success:
		...
		if success:
			...
			step = step + 1
			if(step%interval==0):
				features_arr.append(np.array(features))
				features = []
				count = count -2
	unscaled_features = features_arr
	with open(feature_output_file_path, 'wb') as pickle_file:
		pk.dump(unscaled_features,pickle_file)
